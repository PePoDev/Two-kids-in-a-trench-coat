	...
	static class SearchUtils {
		...
		public static void Upd(SearchArg arg) {
			if (arg.Target is DefaultAsset folder) {
				...
				arg.UnusedAssetsFiltered = store.UnusedFiles.Where(p => p.Key.StartsWith(path, StringComparison.InvariantCultureIgnoreCase)).Select(p => p.Key).ToList();
				arg.UnusedScenesFiltered = store.UnusedScenes.Where(p => p.Key.StartsWith(path, StringComparison.InvariantCultureIgnoreCase)).Select(p => p.Key).ToList(); 
			}
		}
		...
		public static void ScenesThatContain(Object activeObject) {
			...
			foreach (var path in dep.Lookup) {
				...
				var alreadyOpened = false;
     			...
     			var loadedSceneCount = UnityEditor.SceneManagement.EditorSceneManager.loadedSceneCount;
#endif
